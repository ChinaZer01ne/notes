...
​	魔数之后的四个字节表示版本号，前两个字节表示次版本号（minjor version），后两个字节表示主版本号（major version）。比如`0x0000 0x0034`代表次版本号为0，主版本号为52（十进制）。所以该文件的版本号为：1.8.0（52对应的是jdk8，51对应7，以此类推）。可以通过`java -version`来验证。低版本的jdk编译在高版本是可以运行的，反之是可以运行的。
