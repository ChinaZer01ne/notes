...
​	第一个线程获取锁，执行业务完成，释放锁，是不会有同步队列产生的。当第一个线程持有锁期间，如果有第二个线程竞争锁，此时第二个线程会竞争失败，然后同步队列才开始初始化，会创建一个头节点，然后将第二个线程封装成Node，追加到头节点之后，此时的头节点并不是持有锁的线程绑定的节点。当第一个线程释放锁之后，同步队列会将头节点的下一个节点（第二个线程绑定的节点）unpark唤醒，并设置为头节点，然后将头节点出队，之后的头节点就都是持有锁线程绑定的节点了。
