...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    /**
	 * 与统计数目有关，可以参考jdk1.8新引入的java.util.concurrent.atomic.LongAdder的源码，帮助理解
	 ...
	 * 源码的383-384行写了个说明：A generation stamp in field sizeCtl ensures that resizings do not overlap.
    ...
    public V get(Object key) {
        ...
        if ((tab = table) != null && (n = tab.length) > 0 &&
            ...
            while ((e = e.next) != null) {	// 是普通节点，使用链表方式查找
                if (e.hash == h &&
                    ((ek = e.key) == key || (ek != null && key.equals(ek))))	// 这个条件在我写的678的HashMap中说过几次了，这里就不说了
                    return e.val;
            }
        }
        return null;
        ...
        for (Node<K,V>[] tab = table;;) {
            ...
            if (tab == null || (n = tab.length) == 0)	// 处理初始化，并发的情况在initTable中处理，这里不考虑
                ...
                if (casTabAt(tab, i, null,
                             new Node<K,V>(hash, key, value, null)))	// 使用CAS添加第一个节点
...
// 上面说了，size方法可能会超出int型，而返回不正确的结果，这个方法就是用来替代size的，1.8才新增的方法
    ...
    /**
	 * 保留节点
	 * 或者叫空节点，computeIfAbsent和compute这两个函数式api中才会使用。
	 * 它的hash值固定为-3，就是个占位符，不会保存实际的数据，正常情况是不会出现的，
	 * 在jdk1.8新的函数式有关的两个方法computeIfAbsent和compute中才会出现。
	 *
	 * 为什么需要这个节点？
	 * 因为正常的写操作，都会想对hash桶的第一个节点进行加锁，但是null是不能加锁，
	 * 所以就要new一个占位符出来，放在这个空hash桶中成为第一个节点，把占位符当锁的对象，
	 * 这样就能对整个hash桶加锁了。
	 * put/remove不使用ReservationNode是因为它们都特殊处理了下，并且这种特殊情况实际上还更简单，
    ...
    final Node<K,V>[] helpTransfer(Node<K,V>[] tab, Node<K,V> f) {
        ...
        if (tab != null && (f instanceof ForwardingNode) &&
            ...
            while (nextTab == nextTable && table == tab &&
                   (sc = sizeCtl) < 0) {	 // 在判断一次是否正在执行扩容（这几个变量的值改变了，说明此次扩容结束了）
                if ((sc >>> RESIZE_STAMP_SHIFT) != rs || sc == rs + 1 ||
...
// 用于putAll，此时是需要考虑初始化；链表转化为红黑树中，不满足table容量条件时，进行一次扩容，此时就是普通的扩容
    ...
    /* ---------------- TreeBins -------------- */
	/**
	 * TreeBin的hash值固定为-2，它是ConcurrentHashMap中用于代理操作TreeNode的特殊节点，
	 * 持有存储实际数据的红黑树的根节点。因为红黑树进行写入操作，整个树的结构可能会有很大的变化，
	 * 这个对读线程有很大的影响，所以TreeBin还要维护一个简单读写锁，这是相对HashMap，
	 * 这个类新引入这种特殊节点的重要原因。
	 *
