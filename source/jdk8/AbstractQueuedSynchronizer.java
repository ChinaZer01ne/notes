...
public abstract class AbstractQueuedSynchronizer
    ...
    /**
	 * 等待队列的头，延迟初始化。除了初始化，它正能通过setHead方法修改。
	 * 注意:如果head存在，它的等待状态不是CANCELLED
    ...
    private Node enq(final Node node) {
        for (;;) {
            Node t = tail;
            if (t == null) { // Must initialize 必须初始化，当同步队列为空时，head == tail == null，
							//这个if并没有返回值，只是做了一个初始化，下一次自旋，才真正的插入值
    ...
    private Node addWaiter(Node mode) {	//mode表示该节点的共享/排他性，null为排他，不为null为共享
		// 初始化一个Node，保存了当前线程信息和MODE
        Node node = new Node(Thread.currentThread(), mode);
        // Try the fast path of enq; backup to full enq on failure
        Node pred = tail;
        if (pred != null) {	//尾节点不为空，新节点连到队列尾部，尾插法
            node.prev = pred;
        ...
        enq(node);	//如果尾节点为空，通过enq方法入队（包含了队列初始化的逻辑）
        return node;
    }
    ...
    private void unparkSuccessor(Node node) {
        ...
        /* 
         × 要唤醒的节点通常就是下一个节点，如果这个节点是CACELLED状态或者为null，
         × 则从tail节点往前遍历寻找非CACELLED状态的节点（这个循环没有break,node接下来的第一个非CACELLED状态的节点）
    ...
    /**
     × 共享模式的释放动作，信号继承并确保传播。（注意：对于独占模式，如果需要信号，释放就等于调用head的unparksuccessor（唤醒Head的下一个节点）。）
    ...
    private void doReleaseShared() {
        ...
        for (;;) {
            ...
            if (h != null && h != tail) {
                ...
                else if (ws == 0 &&		// 如果waitStatus == 0,则将waitStatus变为PROPAGATE
    ...
    private void setHeadAndPropagate(Node node, int propagate) {
        Node h = head; // Record old head for check below	//记录当前头节点
