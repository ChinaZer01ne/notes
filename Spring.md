...
| 事务传播行为类型          | 说明                                                         |
| ------------------------- | ------------------------------------------------------------ |
| PROPAGATION_REQUIRED      | 如果当前没有事务，就新建一个事务，如果已经存在一个事务中，加入到这个事务中。这是最常见的选择。    理解：**如果调用者没有事务，就新建一个事务，如果有，那么被调用者事务加入调用者事务，也就是说，必须有一个事务，** |
| PROPAGATION_SUPPORTS      | 支持当前事务，如果当前没有事务，就以非事务方式执行。理解：**支持调用者的事务，如果调用者没有事务，那么被调用者也不使用事务** |
| PROPAGATION_MANDATORY     | 使用当前的事务，如果当前没有事务，就抛出异常。理解：**使用被调用者的事务，如果调用者没有事务，就抛出异常** |
| PROPAGATION_REQUIRES_NEW  | 新建事务，如果当前存在事务，把当前事务挂起。 理解：**使用被调用者的事务，无视调用者的事务** |
